...
class EnhancedVideoDecryptor {
    ...
    async init(videoId) {
        try {
            ...
            console.log('🗺️ Raw chunk map:', JSON.stringify(this.qualityChunkMap, null, 2));

            // Auto-select best quality
            this.currentQuality = this.selectBestQuality();

            // Initialize quality state
            await this.initializeQualityState(this.currentQuality, this.currentAudioTrack);
    ...
    async initializeQualityState(quality, audioTrack) {
        ...
        if (chunkMap.audio) {
            ...
            if (Array.isArray(chunkMap.audio)) {
                ...
                const audioInfo = chunkMap.audio.find(a => a.stream_id === (audioTrack + 1));
                if (audioInfo) {
                ...
                let audioData = chunkMap.audio[streamId] || chunkMap.audio[streamIdStr];
                
                if (audioData) {
                    // OLD FORMAT: has "chunks" array
                    if (audioData.chunks && Array.isArray(audioData.chunks)) {
                        audioChunkCount = audioData.chunks.length;
                        console.log(`   ⚠️ OLD FORMAT detected for stream ${streamId}: ${audioChunkCount} chunks in array`);
                    }
                    // NEW FORMAT: has "count" property
                    ...
                    if (firstKey && chunkMap.audio[firstKey]) {
                        const firstAudioData = chunkMap.audio[firstKey];
                        
                        // OLD FORMAT
                        if (firstAudioData.chunks && Array.isArray(firstAudioData.chunks)) {
                            audioChunkCount = firstAudioData.chunks.length;
                            console.log(`   ⚠️ OLD FORMAT (fallback key ${firstKey}): ${audioChunkCount} chunks in array`);
                        }
                        // NEW FORMAT
                        else if (firstAudioData.count) {
                            audioChunkCount = firstAudioData.count;
                        }
                    }
                }
            }
    ...
    isChunkProcessed(type, quality, track, index) {
        ...
        return state.status === 'loaded' || state.status === 'skipped' || state.status === 'loading';
    }
    ...
    async initSourceBuffers() {
        ...
        // Add event listeners
        this.videoBuffer.addEventListener('updateend', () => this.onBufferUpdateEnd('video'));
        this.videoBuffer.addEventListener('error', (e) => console.error('Video buffer error:', e));
        
        this.audioBuffer.addEventListener('updateend', () => this.onBufferUpdateEnd('audio'));
        this.audioBuffer.addEventListener('error', (e) => console.error('Audio buffer error:', e));
    ...
    async appendInitSegments() {
        ...
        try {
            const [vInit, aInit] = await Promise.all([
                ...
                this.fetchInitSegment('audio', this.currentQuality, this.currentAudioTrack)
            ]);
    ...
    async manageChunkLoading() {
        ...
        if (chunksAhead < targetChunksAhead) {
            ...
            // Find the next chunk to load - start from current chunk
            // and skip already-loaded chunks
            let chunksLoaded = 0;
            let checkIndex = currentChunk;
            
            while (chunksLoaded < chunksToLoad) {
                // Check bounds - stop only if BOTH streams are exhausted
                const videoExhausted = checkIndex >= this.currentQualityState.maxVideoChunks;
                const audioExhausted = checkIndex >= this.currentQualityState.maxAudioChunks;
                
                // If both streams are exhausted, end stream
                ...
                const videoLoaded = this.isChunkProcessed('video', this.currentQuality, 0, checkIndex);
                const audioLoaded = this.isChunkProcessed('audio', this.currentQuality, this.currentAudioTrack, checkIndex);
                
                // If not fully loaded, load it
    ...
    async loadChunkPair(chunkIndex) {
        ...
        const videoProcessed = this.isChunkProcessed('video', quality, 0, chunkIndex);
        const audioProcessed = this.isChunkProcessed('audio', quality, audioTrack, chunkIndex);
        ...
        const audioPromise = this.loadSingleChunk('audio', quality, audioTrack, chunkIndex);
        ...
        if (videoData) {
            ...
            if (!this.processingQueue && !this.videoBuffer.updating && !this.audioBuffer.updating) {
    ...
    async loadSingleChunk(type, quality, track, index) {
        ...
        if (state.retries >= this.config.maxRetries) {
            ...
            this.setChunkState(type, quality, track, index, { status: 'skipped', retries: state.retries });
            return null;
        }

        // Check bounds
        const maxChunks = type === 'video' ? 
            this.currentQualityState.maxVideoChunks : 
            this.currentQualityState.maxAudioChunks;

        // If no audio chunks exist at all, skip audio loading
        if (type === 'audio' && maxChunks === 0) {
            this.setChunkState(type, quality, track, index, { status: 'skipped', retries: 0 });
            return null;
        }
        ...
        // Mark as loading
        this.setChunkState(type, quality, track, index, { status: 'loading', retries: state.retries });

        try {
            const data = await this.fetchAndDecryptChunk(type, quality, track, index);
            
            // Mark as loaded
            this.setChunkState(type, quality, track, index, { status: 'loaded', retries: 0 });
            ...
            if (state.retries < this.config.maxRetries) {
                ...
                console.log(`🔁 Retrying ${type} chunk ${index} in ${delay}ms (attempt ${state.retries + 1})`);
        ...
        try {
            while (this.chunkQueue.length > 0) {
                ...
                if (chunk.audioTrack !== this.currentAudioTrack) {
                    console.log(`⏭️ Skipping chunk from old audio track: ${chunk.audioTrack}`);
                    this.chunkQueue.shift();
                    continue;
                }
    ...
    async switchAudioTrack(trackIndex) {
        ...
        try {
            ...
            this.chunkQueue = this.chunkQueue.filter(c => c.audioTrack === this.currentAudioTrack);
        ...
        try {
            if (this.videoBuffer && this.videoBuffer.buffered.length > 0) {
                ...
                for (let i = this.videoBuffer.buffered.length - 1; i >= 0; i--) {
                    ...
                    const end = this.videoBuffer.buffered.end(i);
                    if (end > start) {
                        console.log(`🗑️ Clearing video buffer range ${i}: ${start.toFixed(2)}s - ${end.toFixed(2)}s`);
                        this.videoBuffer.remove(start, end);
                        await this.waitForBufferUpdate(this.videoBuffer);
                    }
                }
            ...
            if (this.audioBuffer && this.audioBuffer.buffered.length > 0) {
                ...
                for (let i = this.audioBuffer.buffered.length - 1; i >= 0; i--) {
                    ...
                    const end = this.audioBuffer.buffered.end(i);
                    if (end > start) {
                        console.log(`🗑️ Clearing audio buffer range ${i}: ${start.toFixed(2)}s - ${end.toFixed(2)}s`);
                        this.audioBuffer.remove(start, end);
                        await this.waitForBufferUpdate(this.audioBuffer);
                    }
                }
    ...
    async seek(timeSeconds) {
        ...
        try {
            ...
            // Restart loader
            setTimeout(async () => {
                console.log('🔄 Seek timeout callback started, wasPlaying:', wasPlaying);
                this.startChunkLoader();
                this.isSeeking = false;
                ...
                if (wasPlaying) {
                    ...
                    // Wait a bit for chunks to load before playing
                    ...
                    console.log('📊 Video currentTime:', videoElement.currentTime.toFixed(2));
                    ...
                    if (videoBuffer && videoBuffer.buffered.length > 0) {
                        console.log('📊 Video buffer:', 
                            ...
                            videoBuffer.buffered.end(videoBuffer.buffered.length - 1).toFixed(2));
                    } else {
                    ...
                    if (audioBuffer && audioBuffer.buffered.length > 0) {
                        console.log('📊 Audio buffer:', 
                            ...
                            audioBuffer.buffered.end(audioBuffer.buffered.length - 1).toFixed(2));
                    } else {
                    ...
                    try {
                        ...
                        setTimeout(() => {
                            console.log('🔍 After play - paused:', videoElement.paused, 'time:', videoElement.currentTime.toFixed(2));
                        }, 100);
                    } catch (err) {
