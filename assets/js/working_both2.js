...
class EnhancedVideoDecryptor {
        ...
        setTimeout(async () => {
          ...
          const canPlayPromise = new Promise((resolve) => {
            const checkBuffer = () => {
              ...
              if (
                videoBuffer &&
                videoBuffer.buffered.length > 0 &&
                audioBuffer &&
                audioBuffer.buffered.length > 0
              ) {
                for (let i = 0; i < videoBuffer.buffered.length; i++) {
                  const start = videoBuffer.buffered.start(i);
                  const end = videoBuffer.buffered.end(i);
          ...
          for (let attempt = 0; attempt < 5; attempt++) {
            try {
              ...
              console.warn(
                `Quality switch play attempt ${attempt + 1} failed:`,
                err.message,
              );
              if (attempt < 4) {
                if (attempt === 1) {
  ...
  async seek(timeSeconds) {
    ...
    try {
      ...
      // CRITICAL: Snap seek time to chunk boundary
      // This ensures video buffer timestamps align with seek position
      const targetChunk = Math.floor(timeSeconds / chunkDuration);
      const alignedTime = targetChunk * chunkDuration;

      console.log(
        `🎯 Seeking to chunk ${targetChunk}, aligned time: ${alignedTime.toFixed(2)}s (requested: ${timeSeconds.toFixed(2)}s)`,
      );

      // Set time to chunk boundary
      videoElement.currentTime = alignedTime;
      ...
      setTimeout(async () => {
        ...
        if (wasPlaying) {
          console.log("⏳ Waiting for buffer at playhead position...");

          // Create promise that resolves when we can play
          const canPlayPromise = new Promise((resolve) => {
            const checkBuffer = () => {
              // Check if we have buffer at current position
              const videoBuffer = this.mediaSource.sourceBuffers[0];
              const audioBuffer = this.mediaSource.sourceBuffers[1];
              const currentTime = videoElement.currentTime;

              let hasVideoBuffer = false;
              ...
              if (videoBuffer && videoBuffer.buffered.length > 0) {
                for (let i = 0; i < videoBuffer.buffered.length; i++) {
                  const start = videoBuffer.buffered.start(i);
                  const end = videoBuffer.buffered.end(i);
                  // Check if currentTime is within this buffered range (with 0.5s tolerance)
                  if (currentTime >= start - 0.5 && currentTime < end) {
                    hasVideoBuffer = true;
                    break;
                  }
              ...
              if (audioBuffer && audioBuffer.buffered.length > 0) {
                for (let i = 0; i < audioBuffer.buffered.length; i++) {
                  const start = audioBuffer.buffered.start(i);
                  const end = audioBuffer.buffered.end(i);
                  if (currentTime >= start - 0.5 && currentTime < end) {
                    hasAudioBuffer = true;
                    break;
                  }
              ...
              if (hasVideoBuffer && hasAudioBuffer) {
                console.log(
                  `✅ Buffer available at ${currentTime.toFixed(2)}s`,
                );
                resolve();
                return true;
              }
            ...
            // Also poll for buffer availability
            let attempts = 0;
            ...
            const interval = setInterval(() => {
              ...
              if (checkBuffer()) {
                ...
                clearInterval(interval);
                videoElement.removeEventListener("canplay", onCanPlay);
                console.warn("⚠️ Buffer check timeout, proceeding anyway");
                resolve();
              }
            }, 100);
          });
          ...
          if (videoBuffer && videoBuffer.buffered.length > 0) {
            const bufferStart = videoBuffer.buffered.start(0);
            const bufferEnd = videoBuffer.buffered.end(
              videoBuffer.buffered.length - 1,
            );
            console.log(
              "📊 Video buffer:",
              bufferStart.toFixed(2),
              "-",
              bufferEnd.toFixed(2),
            );

            // CRITICAL FIX: If currentTime is before buffer start, adjust it to buffer start
            if (videoElement.currentTime < bufferStart) {
              console.log(
                `⚠️ Adjusting currentTime from ${videoElement.currentTime.toFixed(2)}s to ${bufferStart.toFixed(2)}s (buffer start)`,
              );
              videoElement.currentTime = bufferStart;
            }
          }
          ...
          for (let attempt = 0; attempt < 5; attempt++) {
            try {
              ...
              if (attempt < 4) {
                // Small nudge to force buffer check
                if (attempt === 1) {
                  videoElement.currentTime += 0.001;
                }
                await new Promise((resolve) => setTimeout(resolve, 300));
          ...
          if (!playSuccess) {
            console.error("❌ Failed to resume playback after all retries");
            // Last resort: nudge time forward slightly
            videoElement.currentTime += 0.1;
            try {
              await videoElement.play();
              console.log("✅ Playback resumed after time nudge");
            } catch (err) {
              console.error("❌ Complete failure to resume:", err);
            }
          }
        } else {
